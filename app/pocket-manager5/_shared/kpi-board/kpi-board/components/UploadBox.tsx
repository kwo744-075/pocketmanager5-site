...
import { CloudUpload, FileSpreadsheet, Info, RefreshCw } from "lucide-react";
...
export function UploadBox({ onParsed, defaultSection = "daily" }: UploadBoxProps) {
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-white/10 bg-slate-950/70 p-5 shadow-lg shadow-black/30">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-[10px] uppercase tracking-[0.4em] text-slate-500">Step 1 • Upload</p>
          <h3 className="text-xl font-semibold text-white">Drop your KPI export</h3>
          <p className="text-sm text-slate-300">XLSX only. One upload feeds mapping, goals, and the board preview.</p>
        </div>
      ...
      <div
        className={`flex cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-white/15 bg-slate-900/60 p-8 text-center transition ${
          dragActive ? "border-emerald-400/60 bg-emerald-500/10" : "hover:border-emerald-400/40"
        }`}
        onDragOver={(event) => {
          event.preventDefault();
          setDragActive(true);
      ...
      >
        <CloudUpload className="h-10 w-10 text-emerald-300" />
        <div>
          <p className="text-lg font-semibold text-white">Drag & drop or browse</p>
          <p className="text-sm text-slate-400">XLSX or XLS only. If your report has multiple tabs, pick one below.</p>
        </div>
        <input
          ref={fileInputRef}
          type="file"
      ...
      {sheetNames.length ? (
        <div className="grid gap-4 rounded-2xl border border-white/5 bg-slate-900/60 p-4 md:grid-cols-[1.1fr_1.2fr]">
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm text-slate-300">
              <FileSpreadsheet className="h-4 w-4 text-emerald-200" />
              <span>Pick table region</span>
            </div>
            ...
            <div className="rounded-xl border border-dashed border-white/10 bg-slate-950/70 p-3 text-xs text-slate-300">
              <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.35em] text-slate-500">
                <Info className="h-3.5 w-3.5" />
                <span>Helper</span>
              </div>
              <p className="mt-2">
                We auto-pick the first row that mentions Shop or Store. If that misses, select the header row manually.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Header preview</p>
              <button
                type="button"
                onClick={() => {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-xs text-slate-400">
          <p className="font-semibold text-slate-200">Upload → Map once → See board</p>
          <p>We only parse in-browser. Nothing leaves the page until you save to Supabase.</p>
        </div>
        <button
          type="button"
