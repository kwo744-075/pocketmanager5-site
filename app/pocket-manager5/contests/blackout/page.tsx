...
type MarkedState = Record<string, Set<string>>; // objectiveId -> shops
...
export default function BlackoutPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const session = await getActiveSession("blackout", scope.scope ?? null);
      if (session) setSessionId(session.id);
      const board = await getBoardTemplate("blackout");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const rows = await getMarks("blackout", sessionId);
  ...
  const handleStart = async () => {
    const session = await startSession("blackout", "Blackout Contest", null);
    if (session) setSessionId(session.id);
  };
  ...
  const handleToggle = async (objectiveId: string) => {
    ...
    if (hasMark) {
      await unmark("blackout", sessionId, shopNumber, objectiveId);
    } else {
      await mark("blackout", sessionId, shopNumber, objectiveId);
    }
  };
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 space-y-6">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-semibold text-white">Blackout</h1>
          </div>
