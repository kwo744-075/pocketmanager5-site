...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "create") {
      const { data, error } = await admin.from("employee_shifts").insert([payload]).select("id").maybeSingle();
      if (error) {
    ...
    if (action === "update") {
      ...
      delete updates.id;
      const { data, error } = await admin.from("employee_shifts").update(updates).eq("id", id).select("id").maybeSingle();
      if (error) {
        console.error("employee_shifts update failed", error);
        return NextResponse.json({ error: error.message ?? "Update failed" }, { status: 500 });
      }
      return NextResponse.json({ ok: true, id: (data as any)?.id ?? id });
    }
